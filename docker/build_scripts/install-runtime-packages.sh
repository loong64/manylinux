...
if [ "${AUDITWHEEL_POLICY}" == "manylinux2014" ]; then
	...
	dnf -y install dnf-plugins-core anolis-epao-release
	...
	if [ "${AUDITWHEEL_ARCH}" == "loongarch64" ]; then
		dnf -y install cracklib-dicts dejavu-sans-fonts info langpacks-core-font-en libpciaccess-devel pcre-devel perl-FindBin perl-IO-Socket-SSL perl-Mozilla-CA perl-NDBM_File perl-subs valgrind-devel
	fi
elif [ "${OS_ID_LIKE}" == "debian" ]; then
