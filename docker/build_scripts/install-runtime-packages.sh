...
if [ "${AUDITWHEEL_POLICY}" == "manylinux2014" ]; then
	...
	if [ "${AUDITWHEEL_ARCH}" == "i686" ] || [ "${AUDITWHEEL_ARCH}" == "riscv64" ]; then
		...
		curl -fsSLo /etc/yum.repos.d/AnolisOS-Devel.repo https://github.com/Loongson-Cloud-Community/docker-library/raw/refs/heads/main/openanolis/anolisos/23.4/AnolisOS-Devel.repo
	...
	if [ "${AUDITWHEEL_ARCH}" == "loongarch64" ]; then
		dnf -y install cracklib-dicts dejavu-sans-fonts info langpacks-core-font-en libpciaccess-devel pcre-devel perl-core perl-FindBin perl-IO-Socket-SSL perl-Mozilla-CA perl-NDBM_File perl-subs
	fi
elif [ "${OS_ID_LIKE}" == "debian" ]; then
